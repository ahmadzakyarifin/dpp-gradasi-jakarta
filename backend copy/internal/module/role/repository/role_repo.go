...
import (
	...
	"github.com/ahmadzakyarifin/schoolpay/backend/internal/module/role/entity"
	"github.com/ahmadzakyarifin/schoolpay/backend/internal/module/role/mapper"
	"github.com/ahmadzakyarifin/schoolpay/backend/internal/module/role/model"
...
func NewRoleRepo(db *gorm.DB, redisClient *redis.Client) RoleRepo {
	return &roleRepo{db: db, redisClient: redisClient}
}
...
func invalidateRolePermissionsCache(ctx context.Context, redisClient *redis.Client, roleID uint) {
	if redisClient != nil {
		cacheKey := fmt.Sprintf("role_permissions:%d", roleID)
...
func (r *roleRepo) CreateRole(ctx context.Context, role *entity.Role, permissionIDs []uint) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if len(permissionIDs) > 0 {
			...
			invalidateRolePermissionsCache(ctx, r.redisClient, role.ID)
		}
		return nil
	})
}
...
func (r *roleRepo) DeleteRole(ctx context.Context, id uint) error {
	err := r.db.WithContext(ctx).Where("id = ?", id).Delete(&model.RoleModel{}).Error
	if err == nil {
		invalidateRolePermissionsCache(ctx, r.redisClient, id)
	}
	return err
}
...
func (r *roleRepo) SaveRolePermissions(ctx context.Context, roleID uint, permissionIDs []uint) error {
	...
	invalidateRolePermissionsCache(ctx, r.redisClient, roleID)
	return nil
}
