...
import (
	...
	"github.com/ahmadzakyarifin/schoolpay/backend/internal/module/auth/mapper"
	"github.com/ahmadzakyarifin/schoolpay/backend/internal/module/auth/repository"
	...
	notifservice "github.com/ahmadzakyarifin/schoolpay/backend/internal/module/notification/service"
...
type authService struct {
	...
	n     notifservice.NotificationService
	cfg   *config.Config
}
...
func (s *authService) ForgotPassword(ctx context.Context, req dto.ForgotPasswordRequest, ip string, userAgent string) (string, error) {
	...
	if user.Status != "active" {
		// Tetap return sukses untuk alasan keamanan
		return "", nil
	}
	...
	if err := s.n.SendEmail(ctx, notifdto.SendEmailRequest{
		To:      user.Email,
		Subject: notification.EventToSubject(notification.EventAuthForgotPassword),
		HTML:    html,
		UserID:  &user.ID,
	}); err != nil {
		return "", errors.New("gagal menjadwalkan pengiriman email reset password")
	}
	...
	s.log(ctx, s.r.GetDB(), &activitylogdto.ActivityLogInput{
		...
		EntityID:    &user.ID,
		EntityLabel: user.Name,
		Description: "Link reset password dibuat dan email dikirim ke antrean",
		IPAddress:   ip,
		UserAgent:   userAgent,
		Metadata: map[string]any{
			"email":      user.Email,
