...
import (
	...
	"github.com/ahmadzakyarifin/schoolpay/backend/internal/module/user/service"
...
func (h *UserHandler) GetByID(c *gin.Context) {
	...
	if err != nil {
		helper.ErrorResponse(c, http.StatusBadRequest, "VALIDATION_ERROR", "ID tidak valid", validator.Errors(err))
		return
	}
...
func (h *UserHandler) Create(c *gin.Context) {
	...
	created, err := h.s.Create(c.Request.Context(), req, channel)
	if err != nil {
		helper.HandleServiceError(c, err)
		return
	}

	helper.SuccessResponse(c, http.StatusCreated, "USER_CREATED", "user berhasil dibuat, notifikasi dikirim", gin.H{"user": mapper.UserEntityToResponse(*created)}, nil)
}
...
func (h *UserHandler) ResendNotification(c *gin.Context) {
	...
	if err := h.s.ResendNotification(c.Request.Context(), uint(id), req.Channel); err != nil {
		helper.HandleServiceError(c, err)
		return
	}
...
func (h *UserHandler) BulkResendNotification(c *gin.Context) {
	...
	result, err := h.s.BulkResendNotification(c.Request.Context(), req.IDs, req.Channel)
	if err != nil {
		helper.HandleServiceError(c, err)
		return
	}
...
func (h *UserHandler) Export(c *gin.Context) {
	...
	data, err := h.s.ExportExcel(c.Request.Context(), search, role, status, sort)
	if err != nil {
		helper.HandleServiceError(c, err)
		return
	}
