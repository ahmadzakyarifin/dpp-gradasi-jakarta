...
// RateLimitPerUser membuat rate limiter berdasarkan user login
// dengan periode default 1 menit.
...
// Use membuat middleware rate limiter.
// Semua rule harus lolos agar request dapat diteruskan.
func (r *RateLimiter) Use(scope string, rules ...Rule) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		for _, item := range preparedRules {
			...
			// Rule user hanya boleh dipakai setelah AuthMiddleware.
...
func hash(value string) string {
	...
	// 32 karakter cukup pendek untuk Redis key,
	// tetapi tetap sangat kecil kemungkinan collision.
