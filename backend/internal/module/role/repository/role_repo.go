...
func (r *roleRepo) GetRoleByID(ctx context.Context, id uint) (*entity.Role, error) {
	...
	err := r.db.WithContext(ctx).Where("roles.id = ?", id).First(&m).Error
	if err != nil {
		return nil, err
	}
...
func (r *roleRepo) GetRoleByIDUnscoped(ctx context.Context, id uint) (*entity.Role, error) {
	var m model.RoleModel
	err := r.db.WithContext(ctx).Unscoped().Where("roles.id = ?", id).First(&m).Error
	if err != nil {
		return nil, err
	}
	return mapper.ModelToEntity(&m), nil
...
func (r *roleRepo) CreateRole(ctx context.Context, role *entity.Role) error {
...
func (r *roleRepo) UpdateRole(ctx context.Context, role *entity.Role) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		m := mapper.EntityToModel(role)
		return tx.Model(&model.RoleModel{}).Where("id = ?", m.ID).Updates(map[string]any{
			"name":         m.Name,
